...
// FIXME #include "stm32h7xx_hal.h"			// STM32IPL
// FIXME #include "stm32h7xx_hal_dma2d.h"	// STM32IPL

#ifdef STM32IPL
__attribute__((weak)) void STM32Ipl_FaultHandler(const char *error);
#define nlr_raise(a) STM32Ipl_FaultHandler(a)
...
#define IM_TO_GS_PIXEL(img, x, y) \
    ({uint32_t index = 3 * (y) * img->w + (x); \
	img->bpp == IMAGE_BPP_GRAYSCALE ? img->pixels[((y) * img->w) + (x)] : \
    img->bpp == IMAGE_BPP_RGB565 ? COLOR_RGB565_TO_Y(((uint16_t*)img->pixels)[((y) * img->w) + (x)]) : \
    COLOR_RGB888_TO_Y(((uint8_t*)img->pixels)[index + 2], ((uint8_t*)img->pixels)[index + 1], ((uint8_t*)img->pixels)[index]);})
#endif

// FIXME Can we optimize it?
#define IM_TO_GS_PIXEL(img, x, y)    \
    ( img->bpp == IMAGE_BPP_GRAYSCALE ? img->pixels[((y)*img->w)+(x)] : \
    img->bpp == IMAGE_BPP_RGB565 ? COLOR_RGB565_TO_Y(((uint16_t*)img->pixels)[((y)*img->w)+(x)]) : \
...
// Statistics
		bool imlib_get_regression_points(find_lines_list_lnk_data_t *out, point_t* ptr, uint16_t n_points, bool robust); // STM32IPL
