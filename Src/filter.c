    ...
    switch(img->bpp) {
		...
		case IMAGE_BPP_RGB888: { // STM32IPL
			...
			for (int y = 0, yy = img->h; y < yy; y++) {
				...
				for (int x = 0, xx = img->w; x < xx; x++) {
					rgb888_t pixel = IMAGE_GET_RGB888_PIXEL_FAST(row_ptr, x);
					hist[ COLOR_RGB888_TO_Y(pixel.r, pixel.g, pixel.b ) - COLOR_Y_MIN] += 1;
				}
			}
			...
			for (int y = 0, yy = img->h; y < yy; y++) {
				...
				for (int x = 0, xx = img->w; x < xx; x++) {
					...
					pixel = imlib_yuv_to_rgb888(fast_floorf(s * hist[y]), u, v);
        ...
        case IMAGE_BPP_RGB888: {
            ...
            for (int y = 0, yy = img->h; y < yy; y++) {
                ...
                for (int x = 0, xx = img->w; x < xx; x++) {
                    ...
                    if (!mask && x > ksize && x < img->w-ksize && y >= ksize && y < img->h-ksize) {
                       for (int j= -ksize; j<=ksize; j++) {
                    	   rgb888_t *k_row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, y+j);
                           // subtract last left-most pixel from the sums
                           pixel888 = IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr, x-ksize-1);
                           r_acc -= pixel888.r;
                           g_acc -= pixel888.g;
                           b_acc -= pixel888.b;
                           // add new right edge pixel to the sums
                           pixel888 = IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr, x+ksize);
                           r_acc += pixel888.r;
                           g_acc += pixel888.g;
                           b_acc += pixel888.b;
                        }
                        ...
                        r_acc = g_acc = b_acc = 0;
                        for (int j = -ksize; j <= ksize; j++) {
                        	rgb888_t *k_row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img,
                            IM_MIN(IM_MAX(y + j, 0), (img->h - 1)));

                            for (int k = -ksize; k <= ksize; k++) {
                                pixel888 = IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr,
                                    IM_MIN(IM_MAX(x + k, 0), (img->w - 1)));
                                r_acc += pixel888.r;
                                g_acc += pixel888.g;
                                b_acc += pixel888.b;
...
void imlib_median_filter(image_t *img, const int ksize, float percentile, bool threshold, int offset, bool invert, image_t *mask)
{
    ...
    switch(img->bpp) {
        ...
        case IMAGE_BPP_RGB888: {
            buf.data = fb_alloc(IMAGE_RGB888_LINE_LEN_BYTES(img) * brows, FB_ALLOC_NO_HINT);
            uint8_t *r_data = fb_alloc(64, FB_ALLOC_NO_HINT);
            uint8_t *g_data = fb_alloc(64, FB_ALLOC_NO_HINT);
            uint8_t *b_data = fb_alloc(64, FB_ALLOC_NO_HINT);
            uint8_t r, g, b;
            rgb888_t pixel888;
            for (int y = 0, yy = img->h; y < yy; y++) {
                rgb888_t *row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, y);
                ...
                for (int x = 0, xx = img->w; x < xx; x++) {
                    ...
                    if (!mask && x > ksize && x < img->w-ksize && y >= ksize && y < img->h-ksize) {
                        for (int j = -ksize; j <= ksize; j++) {
                            rgb888_t *k_row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, y+j);
                            pixel888 = IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr, x-ksize-1);
                            r_data[pixel888.r >> 2]--; // remove left pixel
                            g_data[pixel888.g >> 2]--;
                            b_data[pixel888.b >> 2]--;
                            pixel888 = IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr, x+ksize);
                            r_data[pixel888.r >> 2]++; // add right pixel
                            g_data[pixel888.g >> 2]++;
                            b_data[pixel888.b >> 2]++;
                        }
                    } else { // need to check bounds
                        memset(r_data, 0, 64); memset(g_data, 0, 64); memset(b_data, 0, 64);
                        for (int j = -ksize; j <= ksize; j++) {
                            rgb888_t *k_row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img,
                                IM_MIN(IM_MAX(y + j, 0), (img->h - 1)));
                    ...
                    r = hist_median(r_data, 64, median_cutoff);
                    g = hist_median(g_data, 64, median_cutoff);
                    b = hist_median(b_data, 64, median_cutoff);
        ...
        case IMAGE_BPP_RGB888: {
            ...
            for (int y = 0, yy = img->h; y < yy; y++) {
                ...
                if (!mask && x > ksize && x < img->w-ksize && y >= ksize && y < img->h-ksize) {
                    ...
                    memset(r_bins, 0, (COLOR_R8_MAX-COLOR_R8_MIN+1));
                    memset(g_bins, 0, (COLOR_G8_MAX-COLOR_G8_MIN+1));
                    memset(b_bins, 0, (COLOR_B8_MAX-COLOR_B8_MIN+1));
                    r_mcount = g_mcount = b_mcount = 0;
                    for (int j = -ksize; j <= ksize; j++) {
                        rgb888_t *k_row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img,
                            IM_MIN(IM_MAX(y + j, 0), (img->h - 1)));
                ...
                for (int x = 0, xx = img->w; x < xx; x++) {
                    ...
                    if (x >= ksize && x < img->w-ksize && y >= ksize && y < img->h-ksize) {
                        for (int j = -ksize; j <= ksize; j++) {
                            rgb888_t *k_row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img,y+j);
                            for (int k = -ksize; k <= ksize; k++) {
                            	rgb888_t pixel888 = IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr,x+k);
                                int r_pixel = pixel888.r;
                                int g_pixel = pixel888.g;
                                int b_pixel = pixel888.b;
                                if (r_pixel < r_min) r_min = r_pixel;
                                else if (r_pixel > r_max) r_max = r_pixel;
                                if (g_pixel < g_min) g_min = g_pixel;
                                else if (g_pixel > g_max) g_max = g_pixel;
                        ...
                        for (int j = -ksize; j <= ksize; j++) {
                            ...
                            for (int k = -ksize; k <= ksize; k++) {
                            	rgb888_t pixel888 = IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr,
                                    IM_MIN(IM_MAX(x + k, 0), (img->w - 1)));
                                int r_pixel = pixel888.r;
                                int g_pixel = pixel888.g;
                                int b_pixel = pixel888.b;
                    ...
                    if (threshold) {
                        if (((COLOR_RGB888_TO_Y(pixel888.r, pixel888.g, pixel888.b ) - offset) < COLOR_RGB888_TO_GRAYSCALE(IMAGE_GET_RGB888_PIXEL_FAST(row_ptr, x))) ^ invert) {
                            pixel888.r = COLOR_R8_MAX; /* TODO create COLOR_RGB888_BINARY_MAX or COLOR_R8_BINARY_MAX , COLOR_G8_BINARY_MAX, COLOR_B8_BINARY_MAX*/
                            pixel888.g = COLOR_G8_MAX;
                            pixel888.b = COLOR_B8_MAX;

                        } else {
                            pixel888.r = COLOR_R8_MIN; /* TODO create COLOR_RGB888_BINARY_MIN or COLOR_R8_BINARY_MIN , COLOR_G8_BINARY_MIN, COLOR_B8_BINARY_MIN*/
                            pixel888.g = COLOR_G8_MIN;
                            pixel888.b = COLOR_B8_MIN;                        }
                    }
                ...
                for (int x = 0, xx = img->w; x < xx; x++) {
                    ...
                    if (x >= ksize && x < img->w-ksize && y >= ksize && y < img->h-ksize) {
                        for (int j = -ksize; j <= ksize; j++) {
                        	rgb888_t *k_row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img,y+j);
                            for (int k = -ksize; k <= ksize; k++) {
                            	rgb888_t pixel = IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr,x+k);
                                r_acc += krn[ptr] * pixel.r;
                                g_acc += krn[ptr] * pixel.g;
                                b_acc += krn[ptr++] * pixel.b;
                            }
                        }
                    } else {
                        for (int j = -ksize; j <= ksize; j++) {
                        	rgb888_t *k_row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img,
                                IM_MIN(IM_MAX(y + j, 0), (img->h - 1)));
                            for (int k = -ksize; k <= ksize; k++) {
                            	rgb888_t pixel = IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr,
                                    IM_MIN(IM_MAX(x + k, 0), (img->w - 1)));
                                r_acc += krn[ptr] * pixel.r;
                                g_acc += krn[ptr] * pixel.g;
                                b_acc += krn[ptr++] * pixel.b;
...
void imlib_bilateral_filter(image_t *img, const int ksize, float color_sigma, float space_sigma, bool threshold, int offset, bool invert, image_t *mask)
{
    ...
    switch(img->bpp) {
        ...
        case IMAGE_BPP_RGB888: {
            buf.data = fb_alloc(IMAGE_RGB888_LINE_LEN_BYTES(img) * brows, FB_ALLOC_NO_HINT);
            float *r_gi_ptr = fb_alloc((COLOR_R8_MAX - COLOR_R8_MIN + 1) * sizeof(float) *2, FB_ALLOC_NO_HINT);
            float *g_gi_ptr = fb_alloc((COLOR_G8_MAX - COLOR_G8_MIN + 1) * sizeof(float) *2, FB_ALLOC_NO_HINT);
            float *b_gi_ptr = fb_alloc((COLOR_G8_MAX - COLOR_G8_MIN + 1) * sizeof(float) *2, FB_ALLOC_NO_HINT);
            float *r_gi_lut = &r_gi_ptr[256]; // center
            float *g_gi_lut = &g_gi_ptr[256]; // center
            ...
            for (int y = 0, yy = img->h; y < yy; y++) {
                ...
                for (int x = 0, xx = img->w; x < xx; x++) {
                    ...
                    rgb888_t this_pixel888 = IMAGE_GET_RGB888_PIXEL_FAST(row_ptr, x);
                    int r_this_pixel = this_pixel888.r;
                    int g_this_pixel = this_pixel888.g;
                    int b_this_pixel = this_pixel888.b;
                    float r_i_acc = 0, r_w_acc = 0;
                    float g_i_acc = 0, g_w_acc = 0;
                    float b_i_acc = 0, b_w_acc = 0;
                    int ptr = 0;
                    if (x >= ksize && x < img->w-ksize && y >= ksize && y < img->h-ksize) {
                        for (int j = -ksize; j <= ksize; j++) {
                            ...
                            for (int k = -ksize; k <= ksize; k++) {
                                ...
                                float g_w = g_gi_lut[(g_this_pixel - g_pixel)] * gs;
                                float b_w = b_gi_lut[(b_this_pixel - b_pixel)] * gs;
                                r_i_acc += r_pixel * r_w;
                                r_w_acc += r_w;
                                g_i_acc += g_pixel * g_w;
                                g_w_acc += g_w;
                    ...
                    rgb888_t pixel888;
                    pixel888.r = fast_floorf(IM_MIN(IM_DIV(r_i_acc, r_w_acc), COLOR_R8_MAX));
                    pixel888.g = fast_floorf(IM_MIN(IM_DIV(g_i_acc, g_w_acc), COLOR_G8_MAX));
                    pixel888.b = fast_floorf(IM_MIN(IM_DIV(b_i_acc, b_w_acc), COLOR_B8_MAX));
