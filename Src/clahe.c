...
#ifndef BYTE_IMAGE // STM32IPL: this condition is always false when BYTE_IMAGE is defined, so, to avoid IAR warning, it has been commented.
    ...
    switch(img->bpp) {
		...
		case IMAGE_BPP_RGB888: { // STM32IPL
			for (int y = 0, yy = img->h; y < yy; y++) {
				...
				for (int x = 0, xx = img->w; x < xx; x++) {
					IMAGE_PUT_GRAYSCALE_PIXEL_FAST(clahe_row_ptr, x + xOffset, COLOR_RGB888_TO_GRAYSCALE(IMAGE_GET_RGB888_PIXEL_FAST(row_ptr, x)));
				}
			}
			break;
		}
				...
				for (int x = 0, xx = img->w; x < xx; x++) {
					...
					IMAGE_PUT_RGB888_PIXEL_FAST(row_ptr, x,
							imlib_yuv_to_rgb888(IMAGE_GET_GRAYSCALE_PIXEL_FAST(clahe_row_ptr, x + xOffset),
									COLOR_RGB888_TO_U(pixel.r, pixel.g, pixel.b), COLOR_RGB888_TO_V(pixel.r, pixel.g, pixel.b)));
				}
			}
			break;
		}
