 ...
 * @brief  STM32 Image Processing Library - matd module
 ******************************************************************************
 * @attention
 *
...
// STM32IPL #define assert(expression)
#define sqrt(x) fast_sqrtf(x)
#define sqrtf(x) fast_sqrtf(x)
// STM32IPL #define floor(x) fast_floorf(x)
...
// STM32IPL #define fabs(x) fast_fabsf(x)
...
{
	assert(m != NULL);assert(!matd_is_scalar(m));assert(row >= 0);assert(row < m->nrows);assert(col >= 0);assert(col < m->ncols);
...
void matd_put(matd_t *m, int row, int col, TYPE value)
{
	...
	assert(row >= 0);assert(row < m->nrows);assert(col >= 0);assert(col < m->ncols);
...
{
	assert(m != NULL);assert(matd_is_scalar(m));
...
matd_t* matd_select(const matd_t *a, int r0, int r1, int c0, int c1)
{
	...
	assert(r0 >= 0 && r0 < a->nrows);assert(c0 >= 0 && c0 < a->ncols);
...
{
	assert(a != NULL);assert(b != NULL);assert(a->nrows == b->nrows);assert(a->ncols == b->ncols);
...
{
	assert(a != NULL);assert(a->nrows == a->ncols);
	...
	switch (a->nrows) {
		...
		case 3:
			// 3x3 matrix
			return a->data[0] * a->data[4] * a->data[8] - a->data[0] * a->data[5] * a->data[7]
					+ a->data[1] * a->data[5] * a->data[6] - a->data[1] * a->data[3] * a->data[8]
					+ a->data[2] * a->data[3] * a->data[7] - a->data[2] * a->data[4] * a->data[6];
		...
		case 4: {
			...
			return m00 * m11 * m22 * m33 - m00 * m11 * m23 * m32 - m00 * m21 * m12 * m33 + m00 * m21 * m13 * m32
					+ m00 * m31 * m12 * m23 - m00 * m31 * m13 * m22 - m10 * m01 * m22 * m33 + m10 * m01 * m23 * m32
					+ m10 * m21 * m02 * m33 - m10 * m21 * m03 * m32 - m10 * m31 * m02 * m23 + m10 * m31 * m03 * m22
					+ m20 * m01 * m12 * m33 - m20 * m01 * m13 * m32 - m20 * m11 * m02 * m33 + m20 * m11 * m03 * m32
					+ m20 * m31 * m02 * m13 - m20 * m31 * m03 * m12 - m30 * m01 * m12 * m23 + m30 * m01 * m13 * m22
					+ m30 * m11 * m02 * m23 - m30 * m11 * m03 * m22 - m30 * m21 * m02 * m13 + m30 * m21 * m03 * m12;
		}
...
matd_t* matd_inverse(const matd_t *x)
{
	...
	assert(x != NULL);assert(x->nrows == x->ncols);
	...
	// STM32IPL return NULL; // unreachable
}
...
static inline matd_t* matd_op_gobble_right(const char *expr, int *pos, matd_t *acc, matd_t **garb, int *garbpos)
{
	while (expr[*pos] != 0) {
		...
		switch (expr[*pos]) {
			...
			case '^': {
				assert(acc != NULL);assert(expr[*pos+1] == '-');assert(expr[*pos+2] == '1');
...
static matd_t* matd_op_recurse(const char *expr, int *pos, matd_t *acc, matd_t **args, int *argpos, matd_t **garb,
...
//	matd_t *args[nargs];
	matd_t **args = xalloc(nargs * sizeof(matd_t*));
	if (!args)
		return NULL;
...
//	matd_t *garb[2 * exprlen]; // can't create more than 2 new result per character
	...
	matd_t **garb = xalloc(2 * exprlen * sizeof(matd_t*));
	if (!garb) {
		xfree(args);
		return NULL;
	}
...
{
	assert(a != NULL);assert(matd_is_vector(a));
...
{
	assert(a != NULL);assert(b != NULL);assert(matd_is_vector(a) && matd_is_vector(b));assert(a->nrows*a->ncols == b->nrows*b->ncols);
...
{
	/* STM32IPL: useless.
	 assert(a != NULL); assert(b != NULL); assert(matd_is_vector(a) && matd_is_vector(b));
...
{
// STM32IPL: useless. assert(a != NULL);
// STM32IPL: useless. assert(b != NULL);
// STM32IPL: useless. assert(matd_is_vector(a) && matd_is_vector(b));

	int adim = a->ncols * a->nrows;
// STM32IPL: useless. int bdim = b->ncols * b->nrows;
// STM32IPL: useless. assert(adim == bdim);
...
{ // only defined for vecs (col or row) of length 3
	assert(a != NULL);assert(b != NULL);assert(matd_is_vector_len(a, 3) && matd_is_vector_len(b, 3));
...
{
	assert(a->nrows == b->nrows);assert(a->ncols == b->ncols);
...
// RS: cumulative right-handed transformations.
	...
	for (int hhidx = 0; hhidx < A->nrows; hhidx++) {
		...
		if (hhidx < A->ncols) {
			...
			//float v[vlen];
			float *v = xalloc(vlen * sizeof(float));
			if (!v) {
				matd_svd_t res;
...
// maximum absolute value? (used by method 1)
	...
	int *maxrowidx = xalloc(B->ncols * sizeof(int));
	if (!maxrowidx) {
		matd_svd_t res;
		memset(&res, 0, sizeof(res));
		return res;
...
// U/LS.
	//int idxs[A->ncols];
	int *idxs = xalloc(A->ncols * sizeof(int));
	if (!idxs){
		matd_svd_t res;
		memset(&res, 0, sizeof(res));
		return res;
	...
	//float vals[A->ncols];
	float *vals = xalloc(A->ncols * sizeof(float));
	if (!vals){
		matd_svd_t res;
		memset(&res, 0, sizeof(res));
		return res;
...
// only for square matrices.
	...
	for (int j = 0; j < a->ncols; j++) {
		...
		if (p != j) {
			//TYPE tmp[lu->ncols];
			TYPE *tmp = xalloc(lu->ncols * sizeof(TYPE));
			// STM32IPL: as calloc calls above are not checked against memory leakage,
			// this xalloc is not checked as well :-(
