...
// STM32IPL: introduced macros to support different IDEs.
...
void imlib_logpolar_int(image_t *dst, image_t *src, rectangle_t *roi, bool linear, bool reverse)
{
    ...
    const int m_pi_2_0_d_i = (int)m_pi_2_0_d; // STM32IPL: added cast.
    ...
    if (!reverse) {
        ...
        switch (src->bpp) {
            ...
            case IMAGE_BPP_RGB888: { // STM32IPL
                rgb888_t *tmp = (rgb888_t *) src->data;
                int tmp_w = src->w, tmp_h = src->h, tmp_x = roi->x + w_2 - 1, tmp_y = roi->y + h_2;

                for (int y = 0, yy = h; y < yy; y++) {
                	rgb888_t *row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(dst, y);
                    float rho = y * rho_scale;
                    if (!linear) rho = fast_expf(rho);
                    for (int x = 0, xx = w_2; x < xx; x++) {
