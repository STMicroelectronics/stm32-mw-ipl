                    ...
                    for (int x = roi->x + (y % x_stride), xx = roi->x + roi->w, x_max = xx - 1; x < xx; x += x_stride) {
                        ...
                        && COLOR_THRESHOLD_BINARY(IMAGE_GET_BINARY_PIXEL_FAST(row_ptr, x), &lnk_data, invert)) {
                            ...
                            for (int i = 0; i < FIND_BLOBS_CORNERS_RESOLUTION; i++) {
                                corners[i].x = (int16_t)(IM_MAX(IM_MIN(x_max * sign(cos_table[FIND_BLOBS_ANGLE_RESOLUTION*i]), x_max), 0)); // STM32IPL: added cast.
                                corners[i].y = (int16_t)(IM_MAX(IM_MIN(y_max * sign(sin_table[FIND_BLOBS_ANGLE_RESOLUTION*i]), y_max), 0)); // STM32IPL: added cast.
