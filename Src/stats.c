...
void imlib_similarity_line_op(image_t *img, int line, void *other, void *data, bool vflipped)
{
    ...
    switch(img->bpp) {
        ...
        case IMAGE_BPP_RGB888: { // STM32IPL
            rgb888_t *row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, line);
                ...
                if (!other) {
                    ...
                    for (int y = roi->y, yy = roi->y + roi->h; y < yy; y++) {
                        ...
                        for (int x = roi->x, xx = roi->x + roi->w; x < xx; x++) {
                            ...
                            pixel888.g = g;
                            pixel888.b = b;
                            ((uint32_t *) out->LBins)[fast_roundf((COLOR_RGB888_TO_L(pixel888) - COLOR_L_MIN) * l_mult)]++; // needs to be roundf
                            ((uint32_t *) out->ABins)[fast_roundf((COLOR_RGB888_TO_A(pixel888) - COLOR_A_MIN) * a_mult)]++; // needs to be roundf
                            ((uint32_t *) out->BBins)[fast_roundf((COLOR_RGB888_TO_B(pixel888) - COLOR_B_MIN) * b_mult)]++; // needs to be roundf
                        }
                    }
                }
            } else {
                    ...
                    for (list_lnk_t *it = iterator_start_from_head(thresholds); it; it = iterator_next(it)) {
                        ...
                        for (int y = roi->y, yy = roi->y + roi->h; y < yy; y++) {
                            ...
                            for (int x = roi->x, xx = roi->x + roi->w; x < xx; x++) {
                                ...
                                if (COLOR_THRESHOLD_RGB888(pixel888, &lnk_data, invert)) {
                                    ((uint32_t *) out->LBins)[fast_roundf((COLOR_RGB888_TO_L(pixel888) - COLOR_L_MIN) * l_mult)]++; // needs to be roundf
                                    ((uint32_t *) out->ABins)[fast_roundf((COLOR_RGB888_TO_A(pixel888) - COLOR_A_MIN) * a_mult)]++; // needs to be roundf
                                    ((uint32_t *) out->BBins)[fast_roundf((COLOR_RGB888_TO_B(pixel888) - COLOR_B_MIN) * b_mult)]++; // needs to be roundf
                                    pixel_count++;
...
{
    //float cdf[bincount];
	float *cdf = xalloc(bincount * sizeof(float));
	if (!cdf)
		return 0;

    memset(cdf, 0, bincount * sizeof(float));

    //float weighted_cdf[bincount];
    float *weighted_cdf = xalloc(bincount * sizeof(float));
	if (!weighted_cdf)
		return 0;

    memset(weighted_cdf, 0, bincount * sizeof(float));
    ...
    //float variance[bincount];
	float *variance = xalloc(bincount * sizeof(float));
	if (!variance)
		return 0;

    memset(variance, 0, bincount * sizeof(float));
