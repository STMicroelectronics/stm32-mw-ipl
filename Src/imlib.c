...
#include "imlib.h"
#include "common.h"
// STM32IPL #include "omv_boardconfig.h"
...
void point_rotate(int x, int y, float r, int center_x, int center_y, int16_t *new_x, int16_t *new_y)
{
    x -= center_x;
    y -= center_y;
    *new_x = (int16_t)((x * cosf(r)) - (y * sinf(r)) + center_x); // STM32IPL: added cast.
    *new_y = (int16_t)((x * sinf(r)) + (y * cosf(r)) + center_y); // STM32IPL: added cast.
}
...
void point_min_area_rectangle(point_t *corners, point_t *new_corners, int corners_len) // Corners need to be sorted!
{
	...
	uint32_t x1Len = corners_len - 1;

	x1 = xalloc(x1Len * sizeof(int16_t));
	if (!x1)
		return;

	y1 = xalloc(x1Len * sizeof(int16_t));
	if (!y1)
		return;
...
bool lb_clip_line(line_t *l, int x, int y, int w, int h) // line is drawn if this returns true
{
    ...
    int x1_c = (int)(l->x1 + (xdelta * umin)); // STM32IPL: added cast.
    int y1_c = (int)(l->y1 + (ydelta * umin)); // STM32IPL: added cast.
    int x2_c = (int)(l->x1 + (xdelta * umax)); // STM32IPL: added cast.
    int y2_c = (int)(l->y1 + (ydelta * umax)); // STM32IPL: added cast.
    l->x1 = x1_c;
    l->y1 = y1_c;
    l->x2 = x2_c;
    l->y2 = y2_c;
...
// STM32IPL
int8_t imlib_rgb888_to_l(rgb888_t pixel)
{
	float r_lin = xyz_table[pixel.r];
	float g_lin = xyz_table[pixel.g];
	float b_lin = xyz_table[pixel.b];
...
// STM32IPL
int8_t imlib_rgb888_to_a(rgb888_t pixel)
{
	float r_lin = xyz_table[pixel.r];
	float g_lin = xyz_table[pixel.g];
	float b_lin = xyz_table[pixel.b];
...
// STM32IPL
int8_t imlib_rgb888_to_b(rgb888_t pixel)
{
	float r_lin = xyz_table[pixel.r];
	float g_lin = xyz_table[pixel.g];
	float b_lin = xyz_table[pixel.b];
...
rgb888_t imlib_lab_to_rgb888(uint8_t l, int8_t a, int8_t b)
{
	...
	rgb888_t pixel;
	pixel.r = IM_MAX(IM_MIN(fast_floorf(r_lin * COLOR_R8_MAX), COLOR_R8_MAX), COLOR_R8_MIN);
	pixel.g = IM_MAX(IM_MIN(fast_floorf(g_lin * COLOR_G8_MAX), COLOR_G8_MAX), COLOR_G8_MIN);
	pixel.b = IM_MAX(IM_MIN(fast_floorf(b_lin * COLOR_B8_MAX), COLOR_B8_MAX), COLOR_B8_MIN);
...
{
	rgb888_t pixel;
	pixel.r = IM_MAX(IM_MIN(y + ((91881 * v) >> 16), COLOR_R8_MAX), COLOR_R8_MIN);
	pixel.g = IM_MAX(IM_MIN(y - (((22554 * u) + (46802 * v)) >> 16), COLOR_G8_MAX), COLOR_G8_MIN);
	pixel.b = IM_MAX(IM_MIN(y + ((116130 * u) >> 16), COLOR_B8_MAX), COLOR_B8_MIN);
	return pixel;
}
    ...
    if (l >= 4) {
               ...
               &&  ((p[-4] == '.') || (p[-4] == '.'))) {
                    if (IM_IS_JPEG(img) || IM_IS_BAYER(img)) {
                    	// STM32IPL mp_raise_msg(&mp_type_OSError, MP_ERROR_TEXT("Image is not BMP!"));
                    }
                    return FORMAT_BMP;
        } else if (((p[-1] == 'm') || (p[-1] == 'M'))
               &&  ((p[-2] == 'p') || (p[-2] == 'P'))
               ...
               &&  ((p[-4] == '.') || (p[-4] == '.'))) {
                    if (!IM_IS_RGB565(img)) {
                    	// STM32IPL mp_raise_msg(&mp_type_OSError, MP_ERROR_TEXT("Image is not PPM!"));
                    }
                    return FORMAT_PNM;
        } else if (((p[-1] == 'm') || (p[-1] == 'M'))
               &&  ((p[-2] == 'g') || (p[-2] == 'G'))
               ...
               &&  ((p[-4] == '.') || (p[-4] == '.'))) {
                    if (!IM_IS_GS(img)) {
                    	// STM32IPL mp_raise_msg(&mp_type_OSError, MP_ERROR_TEXT("Image is not PGM!"));
                    }
                    return FORMAT_PNM;
        } else if (((p[-1] == 'w') || (p[-1] == 'W'))
               &&  ((p[-2] == 'a') || (p[-2] == 'A'))
               ...
               &&  ((p[-4] == '.') || (p[-4] == '.'))) {
                    if (!IM_IS_BAYER(img)) {
                    	// STM32IPL mp_raise_msg(&mp_type_OSError, MP_ERROR_TEXT("Image is not BAYER!"));
                    }
                    return FORMAT_RAW;
        }
...
bool imlib_read_geometry(FIL *fp, image_t *img, const char *path, img_read_settings_t *rs)
{
    ...
    ||  (magic[1]=='5') || (magic[1]=='6'))) { // PPM
        ...
        file_read_open(fp, path);
        // STM32IPL jpeg_read_geometry(fp, img, path, &rs->jpg_rs);
        // STM32IPL must add the jpeg handling.
    }
...
void imlib_image_operation(image_t *img, const char *path, image_t *other, int scalar, line_op_t op, void *data)
{
    if (path) {
        ...
        if (!IM_EQUAL(img, &temp)) {
            f_close(&fp);
            // STM32IPL mp_raise_msg(&mp_type_OSError, MP_ERROR_TEXT("Images not equal!"));
        }
        ...
        if ((!temp.pixels) || (!temp.h)) {
        	// STM32IPL mp_raise_msg(&mp_type_MemoryError, MP_ERROR_TEXT("Not enough memory available!"));
        }
        ...
        fb_free();
// STM32IPL        #else
// STM32IPL        mp_raise_msg(&mp_type_OSError, MP_ERROR_TEXT("Image I/O is not supported"));
        #endif
    } else
        ...
        switch(img->bpp) {
			...
			case IMAGE_BPP_RGB888: { // STM32IPL
				rgb888_t *row_ptr = fb_alloc(IMAGE_RGB888_LINE_LEN_BYTES(img), FB_ALLOC_NO_HINT);
				rgb888_t scalar888;
				scalar888.r = (scalar >> 16) & 0xFF;
				scalar888.g = (scalar >> 8) & 0xFF;
				scalar888.b = scalar & 0xFF;

				for (int i = 0, ii = img->w; i < ii; i++) {
					IMAGE_PUT_RGB888_PIXEL_FAST(row_ptr, i, scalar888);
				}
...
void imlib_lens_corr(image_t *img, float strength, float zoom, float x_corr, float y_corr)
{
    ...
    int x_off = (int)(w * x_corr); // STM32IPL: added cast.
    int y_off = (int)(h * y_corr); // STM32IPL: added cast.
    ...
    switch(img->bpp) {
		...
		case IMAGE_BPP_RGB888: { // STM32IPL
			...
			for (int y = 0; y < halfHeight; y++) {
				rgb888_t *row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, y);
				rgb888_t *row_ptr2 = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, h - 1 - y);
				int newY = y - halfHeight;
				int newY2 = newY * newY;
				...
				for (int x = 0; x < halfWidth; x++) {
					...
					// top 2 pixels
					if (sourceY_down >= 0 && sourceY_down < h) {
						rgb888_t *ptr = tmp + (w * sourceY_down);
...
int imlib_image_mean(image_t *src, int *r_mean, int *g_mean, int *b_mean)
{
    ...
    switch(src->bpp) {
        ...
        case IMAGE_BPP_RGB888: { // STM32IPL
			for (int i = 0; i < n; i++) {
				rgb888_t p = ((rgb888_t*)src->pixels)[i];
				r_s += p.r;
				g_s += p.g;
				b_s += p.b;
			}
...
int imlib_image_std(image_t *src)
{
    ...
    /* std */
    return (int)(fast_sqrtf(v)); // STM32IPL: added cast.
}
    ...
    for (int y=0; y<img->h-ksize; y++) {
        ...
        if (y > 0) {
            ...
            for (int x=0; x<img->w-ksize; x++) {
                ...
                acc = (int)((acc * m) + b); // scale, offset, and clamp  // STM32IPL: added cast.
