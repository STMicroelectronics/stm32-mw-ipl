...
// STM32IPL #include STM32_HAL_H
#include "stm32h7xx_hal_dma2d.h" // STM32IPL
// STM32IPL #include "dma.h"
#define DMA_BUFFER(p)       (((uint32_t)p & 3) == 0) // STM32IPL
#endif

/* STM32IPL: it seems these are not used anywhere.
...
void imlib_draw_row(int x_start, int x_end, int y_row, imlib_draw_row_data_t *data)
{
    ...
    ({ \
        ...
        int _dst_pixel = (dst_pixel); \
        long _smuad_alpha = (smuad_alpha); \
                      ...
                      imlib_draw_row_callback_t callback, void *dst_row_override)
{
    ...
    || ((hint & IMAGE_HINT_APPLY_COLOR_PALETTE_FIRST) && color_palette)) {
        ...
        imlib_draw_image(&new_src_img, src_img, 0, 0, 1.f, 1.f, NULL, rgb_channel, 256, color_palette, NULL, (image_hint_t)0, NULL, NULL); // STM32IPL added (image_hint_t) cast.
        src_img = &new_src_img;
        rgb_channel = -1;
        color_palette = NULL;
    }
    ...
    imlib_draw_row_data.callback = (void*)callback;    // STM32IPL: cast added.
                    ...
                    do { // Cache the results of getting the source rows
                        ...
                        while (x_not_done) {
                            ...
                            do { // Cache the results of getting the source pixels
                                ...
                                // STM32IPL pixel = __USAT_ASR(pixel, 1, 23);
                                uint32_t sat; // STM32IPL
                                sat = __USAT(pixel, 1); // STM32IPL
                                pixel = sat >> 23; // STM32IPL
...
// in the 8-bit accumulators - the final image looks bad. Might be workable for lower bit-depth images.
                            ...
                            do { // Cache the results of getting the source pixels
                                ...
                                // clamp output
                                // STM32IPL It seems __USAT_ASR is no more available in latest releases of cmsis_gcc.h.
                                // STM32IPL pixel = __USAT_ASR(pixel, 8, 16);
                                uint32_t sat; // STM32IPL
                                sat = __USAT(pixel, 8); // STM32IPL
                                pixel = sat >> 16; // STM32IPL
                    ...
                    do { // Cache the results of getting the source rows
                        ...
                        while (x_not_done) {
                            ...
                            do { // Cache the results of getting the source pixels
                                ...
                                sat = __USAT(g_pixel, 6); // STM32IPL
                                g_pixel = sat >> 16; // STM32IPL

                                long b_pixel = __SMLAD(smuad_dx_dx2, smuad_b_a0_b_a1, (dx3 * b_a2) + b_d1_avg);

                                // clamp output
                                //b_pixel = __USAT_ASR(b_pixel, 5, 16);
                                sat = __USAT(b_pixel, 5); // STM32IPL
                                b_pixel = sat >> 16; // STM32IPL
