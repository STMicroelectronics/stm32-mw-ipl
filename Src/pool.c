...
void imlib_midpoint_pool(image_t *img_i, image_t *img_o, int x_div, int y_div, const int bias)
{
    ...
    switch(img_i->bpp)
    {
        ...
        case IMAGE_BPP_RGB888: { // STM32IPL
            for (int y = 0, yy = img_i->h / y_div, yyy = (img_i->h % y_div) / 2; y < yy; y++) {
                rgb888_t *row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img_o, y);
                for (int x = 0, xx = img_i->w / x_div, xxx = (img_i->w % x_div) / 2; x < xx; x++) {
                    ...
                    rgb888_t pixel888;
                    pixel888.r = ((r_min*min_bias)+(r_max*max_bias))>>8;
                    pixel888.g = ((g_min*min_bias)+(g_max*max_bias))>>8;
                    pixel888.b = ((b_min*min_bias)+(b_max*max_bias))>>8;

                    IMAGE_PUT_RGB888_PIXEL_FAST(row_ptr, x, pixel888);
                }
            }
            break;
        }
        ...
        case IMAGE_BPP_RGB888: { // STM32IPL
            for (int y = 0, yy = img_i->h / y_div, yyy = (img_i->h % y_div) / 2; y < yy; y++) {
                ...
                for (int x = 0, xx = img_i->w / x_div, xxx = (img_i->w % x_div) / 2; x < xx; x++) {
                    ...
                    for (int i = 0; i < y_div; i++) {
                        for (int j = 0; j < x_div; j++) {
                        	pixel888 = IMAGE_GET_RGB888_PIXEL(img_i, xxx + (x * x_div) + j, yyy + (y * y_div) + i);
                            r_acc += pixel888.r;
                            g_acc += pixel888.g;
                            b_acc += pixel888.b;
                        }
