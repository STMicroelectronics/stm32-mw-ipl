...
#define LCD_PIXELFORMAT		DMA2D_OUTPUT_ARGB8888
...
typedef enum
{
	INVALIDATE = 0x01, CLEAN = 0x02
} STM32IPL_DCacheCoherency_t;
...
static uint32_t getInputColorMode(uint32_t format)
{
	switch (format) {
		case IMAGE_BPP_BINARY:
			 ...
			 * will be needed within STM32Ipl_Draw(). */
 ...
 * @retval None
 */
//static void STM32IPL_DCacheCoherencyMaintenance(uint32_t *memAddr, int32_t memSize, STM32IPL_DCacheCoherency_t op)
//{
//	/* Memory address must be aligned to 32 bytes and have size being multiple of 32. */
//	if (((uint32_t)memAddr % 32 != 0) || (memSize % 32 != 0))
//		while (1);
//
//	if (op == INVALIDATE)
//		SCB_InvalidateDCache_by_Addr((void*)memAddr, memSize);
//	else
//	if (op == CLEAN)
//		SCB_CleanDCache_by_Addr((void*)memAddr, memSize);
//}

/*
 * @brief Draws the given image on the screen at the (x,y) coordinates using hardware acceleration.
...
void STM32Ipl_Draw(const image_t *img, uint16_t x, uint16_t y)
{
	...
	/* Coherency purpose: clean the image data area in L1 D-Cache before DMA2D reading. */
	// FIXME STM32IPL_DCacheCoherencyMaintenance((uint32_t*)img->data, STM32Ipl_ImageDataSize(img), CLEAN);

	saveBytesSwap = hlcd_dma2d.Init.BytesSwap;

	uint32_t destination = LCD_FB_ADDR + (y * LCD_WIDTH + x) * LCD_BPP;
	uint32_t source = (uint32_t)img->data;

	hlcd_dma2d.Init.Mode           = DMA2D_M2M_PFC;
	hlcd_dma2d.Init.ColorMode      = LCD_PIXELFORMAT;
	hlcd_dma2d.Init.OutputOffset   = LCD_WIDTH - img->w;
