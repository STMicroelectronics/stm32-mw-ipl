...
static stm32ipl_err_t STM32Ipl_CropScale(image_t *srcImage, image_t *dstImage, rectangle_t *roi, float x_scale,	float y_scale);
static void STM32Ipl_y8_to_rgb565(const uint8_t *src, uint8_t *dst, uint32_t width, uint32_t height);
static void STM32Ipl_rgb565_to_y8(const uint8_t *src, uint8_t *dst, uint32_t width, uint32_t height);
static void STM32Ipl_Diff(image_t *img, const char *path, image_t *other, int scalar, image_t *mask);

/**
 * @brief Initializes the memory manager.
 * @param mem_address	Address of the memory the library will use
 * @param mem_size		Size of the memory in bytes
 * @return void
 */
void STM32Ipl_Init(uint32_t mem_address, uint32_t mem_size)
{
	umm_init((void*)mem_address, mem_size);
	fb_init();
}


/**
 * @brief Un-initializes the memory manager.
 * @return void
 */
...
/**
 * @brief Allocates the data memory of the image.
 * The size of such data memory depends by width, height and bpp (format).
 * Assuming the input image data pointer is null to avoid memory leakage.
 * @param img		Image
 * @param width		Image width
 * @param height	Image height
 * @param bpp		Image bytes per pixel
...
stm32ipl_err_t STM32Ipl_AllocateData(image_t *img, uint32_t width, uint32_t height, image_bpp_t bpp)
{
	...
	data = xalloc(STM32Ipl_DataSize(width, height, bpp));
	if (!data)
	{
...
/**
 * @brief Releases the data memory of the image.
 * @param img	Image
 */
...
/**
 * @brief Copy the source image into another one. Beware the source image data
 * will be shared with the destination image, as no memory is allocated.
 * @param src	The source image
 * @param dst   The destination image (assuming its data pointer is null to avoid memory leakage)
 * @return stm32ipl_err_Ok on success
 */
...
/**
 * @brief Copy the source image data into the destination image data memory.
 * Only the data is actually copied. Source and destination images must have same resolution and format.
 * The destination image data pointer must refer to a valid memory buffer as no new memory is allocated.
 * @param src	The source image
 * @param dst   The destination image (assuming its data pointer refers to a valid buffer)
 * @return stm32ipl_err_Ok on success
 */
...
/**
 * @brief Clone the source image into another one. A new memory buffer is allocated, filled with
 * the source data and assigned to the destination image.
 * @param src	The source image
 * @param dst   The destination image (assuming its input data pointer is null to avoid memory leakage)
 * @return stm32ipl_err_Ok on success
 */
...
/**
 * @brief Get the size of the image data (bytes).
 * @param img	The source image
 * @return the size of the image data (bytes), 0 in case of wrong input.
 */
uint32_t STM32Ipl_ImageDataSize(const image_t *img)
{
	return img ? image_size((image_t*)img) : 0;
}


/**
 * @brief Get the size of the memory needed to store an image with the given properties.
 * @param width		The image width
 * @param height	The image height
 * @param bpp		The image format
 * @return the size of the image data (bytes), 0 in case of wrong input.
 */
...
void STM32Ipl_rgb565_to_y8(const uint8_t *src, uint8_t *dst, uint32_t width, uint32_t height)
{
	uint16_t *rgb = (uint16_t*)src;

	for (uint32_t j = 0; j < height; j++)
	{
		uint32_t offset = j * width;

		for (uint32_t i = 0; i < width; i++)
		{
			*dst++ = COLOR_RGB565_TO_GRAYSCALE(rgb[offset + i]);
		}
	}
}


void STM32Ipl_y8_to_rgb565(const uint8_t *src, uint8_t *dst, uint32_t width, uint32_t height)
{
	...
	for (uint32_t j = 0; j < height; j++)
	{
		...
		for (uint32_t i = 0; i < width; i++)
		{
			*rgb++ = COLOR_GRAYSCALE_TO_RGB565(src[offset + i]);
		}
	}
}
...
/**
 * @brief Converts the source image from its color space to the color space of
 * the destination image. The two images must have the same resolution.
 * The destination image data buffer must be already allocated
 * and have the right size to contain the converted image.
 * @param src	The source image
 * @param dst	The destination image
 * @return	stm32ipl_err_Ok on success
 */
stm32ipl_err_t STM32Ipl_Convert(image_t *src, image_t *dst)
{
	...
	switch (src->bpp)
	{
		case IMAGE_BPP_RGB565:
			switch (dst->bpp)
			{
				...
				case IMAGE_BPP_GRAYSCALE:
					STM32Ipl_rgb565_to_y8(src->data, dst->data, src->w, src->h);
					break;
		...
		case IMAGE_BPP_GRAYSCALE:
			...
			{
				case IMAGE_BPP_RGB565:
					STM32Ipl_y8_to_rgb565(src->data, dst->data, src->w, src->h);
					break;
 ...
 * @param img			The image (grayscale/RGB565)
...
array_t* STM32Ipl_DetectObject(cascade_t *cascade, image_t *img, rectangle_t *roi, float scale_factor, float threshold)
{
	...
	fb_alloc_mark();
	array_t *objects_array = imlib_detect_objects(img, cascade, (roi == NULL) ? &_roi : roi);
	fb_alloc_free_till_mark();
...
stm32ipl_err_t STM32Ipl_CropScale(image_t *srcImage, image_t *dstImage, rectangle_t *roi, float x_scale, float y_scale)
{

	dstImage->w = fast_floorf(roi->w * x_scale);
	if (dstImage->w < 1)
		return stm32ipl_err_InvalidParameter;

	dstImage->h = fast_floorf(roi->h * y_scale);
	if (dstImage->h < 1)
		return stm32ipl_err_InvalidParameter;

	dstImage->bpp = srcImage->bpp;

	dstImage->data = xalloc(image_size(dstImage));
	...
	switch (srcImage->bpp)
	{
		case IMAGE_BPP_BINARY:
		{
			for (int y = 0, yy = dstImage->h; y < yy; y++)
			{
				uint32_t *row_ptr = IMAGE_COMPUTE_BINARY_PIXEL_ROW_PTR(srcImage, fast_floorf(y * over_yscale) + roi->y);
				uint32_t *row_ptr_2 = IMAGE_COMPUTE_BINARY_PIXEL_ROW_PTR(dstImage, y);
				for (int x = 0, xx = dstImage->w; x < xx; x++)
				{
		...
		case IMAGE_BPP_GRAYSCALE:
		{
			for (int y = 0, yy = dstImage->h; y < yy; y++)
			{
				uint8_t *row_ptr = IMAGE_COMPUTE_GRAYSCALE_PIXEL_ROW_PTR(srcImage, fast_floorf(y * over_yscale) + roi->y);
				uint8_t *row_ptr_2 = IMAGE_COMPUTE_GRAYSCALE_PIXEL_ROW_PTR(dstImage, y);
				for (int x = 0, xx = dstImage->w; x < xx; x++)
				{
		...
		case IMAGE_BPP_RGB565:
		{
			for (int y = 0, yy = dstImage->h; y < yy; y++)
			{
				uint16_t *row_ptr = IMAGE_COMPUTE_RGB565_PIXEL_ROW_PTR(srcImage, fast_floorf(y * over_yscale) + roi->y);
				uint16_t *row_ptr_2 = IMAGE_COMPUTE_RGB565_PIXEL_ROW_PTR(dstImage, y);
				for (int x = 0, xx = dstImage->w; x < xx; x++)
				{
...
/**
 * @brief Copy the roi (region of interest) from the Source image to the Destination image. Source and destination must have the same format.
 * @param src_img 	Source image (Binary/Grayscale/RGB565)
 * @param dst_img 	Destination image, image data is allocated by the library
 * @param roi		Region of interest. If Null the function copies source to destination
 * @return stm32ipl_err_Ok on success
 */
stm32ipl_err_t STM32Ipl_Crop(image_t *src_img, image_t *dst_img, rectangle_t *roi)
{
	// Check arguments.
	if (!src_img || !dst_img || !src_img->data)
		return stm32ipl_err_InvalidParameter;
	...
	return STM32Ipl_CropScale(src_img, dst_img, (roi == NULL) ? &_roi : roi, 1.0F, 1.0F);
}


/**
 * @brief Scaling the roi (region of interest) from the Source image to the Destination image. Source and destination must have the same format.
 * @param src_img 	Source image (Binary/Grayscale/RGB565)
 * @param dst_img 	Destination image, image data is allocated by the library
 * @param roi		Region of interest to apply x and y scale
 * @param x_scale	Value by which to scale the image in the x direction.
 * @param y_scale	Value by which to scale the image in the y direction.
 * @return	stm32ipl_err_Ok on success
 */
stm32ipl_err_t STM32Ipl_Scale(image_t *src_img, image_t *dst_img, rectangle_t *roi, float x_scale, float y_scale)
{
	...
	return STM32Ipl_CropScale(src_img, dst_img, (roi == NULL) ? &_roi : roi, x_scale, y_scale);
}
 ...
 * @param img		Input image ( Binary/Grayscale/RGB565 )
 * @param ksize		Kernel size. Use 1 (3x3 kernel), 2 (5x5 kernel), ..., n (((n*2)+1)x((n*2)+1) kernel)
 * @param threshold	if True is enabled adaptive thresholding of the image which sets pixels to one or zero based on a pixel’s brightness in relation to the brightness of the kernel of pixels around them
...
stm32ipl_err_t STM32Ipl_B_nand(image_t* imgA, image_t* imgB, image_t *mask)
{
	...
	int default_val = COLOR_R8_G8_B8_TO_RGB565(IM_MAX(IM_MIN(0, COLOR_R8_MAX), COLOR_R8_MIN),
                                           IM_MAX(IM_MIN(0, COLOR_G8_MAX), COLOR_G8_MIN),
                                           IM_MAX(IM_MIN(0, COLOR_B8_MAX), COLOR_B8_MIN));
    switch(imgA->bpp) {
        case IMAGE_BPP_BINARY: {
                ...
                for (int i = 0, j = img->w; i < j; i++) {
                    if (image_get_mask_pixel(mask, i, line)) {
                        IMAGE_PUT_BINARY_PIXEL_FAST(data, i,
                        		(uint32_t)sqrt(pow(IMAGE_GET_BINARY_PIXEL_FAST(data, i),2)
                             + pow(IMAGE_GET_BINARY_PIXEL_FAST(((uint32_t *) other), i),2)));
                    }
                }
            }
            break;
                ...
                for (int i = 0, j = img->w; i < j; i++) {
                    if (image_get_mask_pixel(mask, i, line)) {
                        IMAGE_PUT_GRAYSCALE_PIXEL_FAST(data, i,
                            sqrt(pow(IMAGE_GET_GRAYSCALE_PIXEL_FAST(data, i),2)
                             + pow(IMAGE_GET_GRAYSCALE_PIXEL_FAST(((uint8_t *) other), i),2)));
                    }
                }
            }
            break;
...
stm32ipl_err_t STM32Ipl_Sobel(image_t *img, uint8_t ksize, bool sharpen)
{
    ...
    */
    ...
    sobel_x.data = xalloc(image_size(img));
	...
	image_init(&sobel_x, img->w, img->h, img->bpp, sobel_x.data);

    sobel_y.data = xalloc(image_size(img));
	...
	image_init(&sobel_y, img->w, img->h, img->bpp, sobel_y.data);

	memcpy(sobel_x.data,img->data,image_size(img));
	memcpy(sobel_y.data,img->data,image_size(img));

	fb_alloc_mark();
	// CMARCH imlib_morph(&sobel_x, ksize, krn, mul, add, threshold, offset, invert, NULL);
        imlib_morph(&sobel_x, ksize, krn, 1.0f / m, 0, false, 0, false, NULL);
    fb_alloc_free_till_mark();
    ...
    imlib_morph(&sobel_y, ksize, krn, 1.0f / m, 0, false, 0, false, NULL);
    fb_alloc_free_till_mark();

    STM32Ipl_Diff(&sobel_x,NULL,&sobel_y,1,NULL);

    xfree(img->data);
    image_init(img, img->w, img->h, sobel_x.bpp, sobel_x.data);
    xfree(sobel_y.data);
	xfree(pascal);
	xfree(krn);
...
stm32ipl_err_t STM32Ipl_Scharr(image_t *img, uint8_t ksize, bool sharpen)
{
    ...
    */
    ...
    scharr_x.data = xalloc(image_size(img));
	if (scharr_x.data == NULL)
	{
		xfree(krn);
		return stm32ipl_err_OutOfMemory;
	}
	image_init(&scharr_x, img->w, img->h, img->bpp, scharr_x.data);

	scharr_y.data = xalloc(image_size(img));
	...
	image_init(&scharr_y, img->w, img->h, img->bpp, scharr_y.data);

	memcpy(scharr_x.data,img->data,image_size(img));
	memcpy(scharr_y.data,img->data,image_size(img));
	...
	// CMARCH imlib_morph(&scharr_x, ksize, krn, mul, add, threshold, offset, invert, NULL);
        imlib_morph(&scharr_x, ksize, krn, 1.0f / m, 0, false, 0, false, NULL);
    fb_alloc_free_till_mark();
    ...
    imlib_morph(&scharr_y, ksize, krn, 1.0f / m, 0, false, 0, false, NULL);
    fb_alloc_free_till_mark();

    STM32Ipl_Diff(&scharr_x,NULL,&scharr_y,1,NULL);

    xfree(img->data);
    image_init(img, img->w, img->h, scharr_x.bpp, scharr_x.data);
    xfree(scharr_y.data);
	xfree(krn);
...
/**
 * @brief Convolves the image by krn kernel kernel.
 * @param img		Input image ( Binary/Grayscale/RGB565 )
 * @param ksize		Kernel size. Use 1 (3x3 kernel), 2 (5x5 kernel), ..., n (((n*2)+1)x((n*2)+1) kernel)
