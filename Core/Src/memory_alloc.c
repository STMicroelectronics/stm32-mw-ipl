...
{
	STM32Ipl_FaultHandler("fb_alloc allocation failure");
}

void umm_alloc_fail(void)
{
	STM32Ipl_FaultHandler("umm_alloc allocation failure");
}
...
void *fb_alloc(uint32_t size, int hints)
{
	...
	p = umm_malloc(size);
	if( p ) g_fb_alloc_stack[ g_fb_alloc_inext++ ] = (uint32_t) p;
	else fb_alloc_fail();
